			...
			if ($object->type != FactureFournisseur::TYPE_CREDIT_NOTE) {
				...
				if ($resql) {
					...
					while ($i < $num) {
						...
						print '<td class="right">';
						// Show unlink discount only when invoice is draft
				...
				// Remainder to pay Multicurrency (hide when multicurrency column already in table)
				...
				print ' :</td>';
				if (!empty($show_multicurrency_column)) {
					print '<td class="right">'.price(price2num($sign * $totalpaid * $object->multicurrency_tx, 'MT'), 0, $langs, 0, -1, -1, (empty($object->multicurrency_code) ? $conf->currency : $object->multicurrency_code)).'</td>';
				}
				print '<td class="right">'.price($sign * $totalpaid, 0, $langs, 0, -1, -1, $conf->currency).'</td><td>&nbsp;</td></tr>';
				...
				print '</td>';
				if (!empty($show_multicurrency_column)) {
					print '<td class="right'.($resteapayeraffiche ? ' amountremaintopay' : (' '.$cssforamountpaymentcomplete)).'">'.price(price2num($sign * $multicurrency_resteapayer, 'MT'), 0, $langs, 0, -1, -1, (empty($object->multicurrency_code) ? $conf->currency : $object->multicurrency_code)).'</td>';
				}
				print '<td class="right'.($resteapayeraffiche ? ' amountremaintopay' : (' '.$cssforamountpaymentcomplete)).'">'.price($sign * $resteapayeraffiche, 0, $langs, 0, -1, -1, $conf->currency).'</td><td>&nbsp;</td></tr>';

				// Remainder to pay back Multicurrency (hide when multicurrency column already in table)
				if (isModEnabled('multicurrency') && ($object->multicurrency_code != $conf->currency || $object->multicurrency_tx != 1) && empty($show_multicurrency_column)) {
					print '<tr><td colspan="'.$nbcols.'" class="right">';
					print '<span class="opacitymedium">';
