...
**移植到 22.0.4**：请阅读上一级目录的 **PORT-TO-22.md**，按「按清单逐文件逻辑移植」策略操作；完成后可生成 `sly22.0-*.patch` 并维护本目录 **APPLY-ON-22.md** 与 **PATCHES-BY-MODULE.md**。

**Core 修改最小化**：请阅读上一级目录的 **CORE-MINIMAL-REVIEW.md**，了解当前全部 core 修改的审查与「最小补丁集」建议；22.0 上应用补丁时见本目录 **APPLY-ON-22.md** 的「最小 Core 应用」小节。

**按功能模块对照**：请阅读本目录 **PATCHES-BY-MODULE.md**，按模块查看 14.0/22.0 补丁对应关系、slycustom 已实现部分及 14.0→22.0 迁移缺口。
...
# 应用单个模块（示例：仅 ShipsGo）
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly14.0-expedition-shipsgo.patch

# 应用多个模块
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly14.0-expedition-shipsgo.patch
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly14.0-compta.patch
# ...
...
```bash
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly14.0-vs-official14.0-FULL.patch
```

FULL 补丁已包含语言包；若只想单独打语言包，可只应用：

```bash
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly14.0-langs.patch
```
