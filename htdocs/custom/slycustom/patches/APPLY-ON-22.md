...
# 3. remx 折扣拆分 hook
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-remx-hooks.patch
...
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-admin.patch

# 10. API：生产模式缓存目录、禁用用户可调 API、文档 ECM/上传简化、CORS/endpoint 规则
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-api.patch

# 11. Misc/Cron/其它（cron 预筛选与 processing=0、去掉 cron 删除确认、delivery generateDocument 默认参数、don setPaid 去掉 paid）
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-misc-cron-other.patch

# 12. 其它模块（adherents、contact、contrat）：会员卡/API 删除/订阅表单、联系人 update_extras 与 delete 触发顺序、合同议程按钮
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-other-modules.patch

# 12b. 银行计划明细（Upcoming entries）：Currency 列、当前/未来余额与 Balance 列对齐、金额不重复显示货币符号
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-bank-treso.patch

# 12c. 客户发票列表：Source order 列紧跟在 Invoice No（Ref）后（需 slycustom 模块）
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-invoice-list-source-order-position.patch

# 12d. 供应商发票列表：Source order 列紧跟在 Ref 后（需 slycustom 模块）
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-supplier-invoice-list-source-order-position.patch
...
git apply --ignore-whitespace htdocs/custom/slycustom/patches/sly22.0-facture-pdf-fallback.patch

# 13. 语言包（en_US、fr_FR、zh_CN）：SLY 翻译与权限键对齐；已排除 22.0 不存在的 zh_CN 文件（externalsite、ftp、link、zapier）
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly22.0-langs.patch
...
4. 测试通过后，可生成 22.0 专用补丁：  
   `git diff upstream/22.0 -- htdocs/模块/ > custom/slycustom/patches/sly22.0-模块.patch`
...
```bash
# 1. 核心（冲突可能很多）
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-core.patch

# 2. 业务模块
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-compta.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-commande.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-fourn.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-comm.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-accountancy-admin-api.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-other-modules.patch
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-misc.patch

# 3. 语言包（可单独应用）
git apply --ignore-whitespace --3way htdocs/custom/slycustom/patches/sly14.0-langs.patch
```
