...
3. 在源代码里 **搜索**（Ctrl+F / Cmd+F）：
   ...
   - **搜不到**：说明当前访问的仍是旧版文件，需要把本地的 `custom/slycustom/admin/setup.php` 部署到服务器。
...
```text
htdocs/custom/slycustom/admin/setup.php
```
...
```bash
grep -n "GETPOST('tab'" custom/slycustom/admin/setup.php
grep -n "SLYCUSTOM_SETUP_TABS_V2" custom/slycustom/admin/setup.php
```
...
- 用带参数的地址直接测 Tab 是否生效：
  - General: `.../custom/slycustom/admin/setup.php?tab=general`
  - PDF: `.../custom/slycustom/admin/setup.php?tab=pdf`
