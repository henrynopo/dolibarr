...
require_once DOL_DOCUMENT_ROOT.'/slycustom/class/langpicker.class.php';
...
if (isModEnabled('slycustom')) {
	...
	$moduleSly->syncMenuPrefixes(); // Ensure Search Order and SLY Exports menu icons in left menu
		...
		$langpickerCss = array('/slycustom/css/langpicker.css.php');
		...
		$langpickerJs = array('/slycustom/js/langpicker.js');
...
if ($action == 'sync_sly_boxes') {
	...
	header('Location: '.$_SERVER["PHP_SELF"].'?tab=boxes');
	exit;
}
...
if ($action == 'sync_sly_pdf_models') {
	...
	header('Location: '.$_SERVER["PHP_SELF"].'?tab=pdf');
	exit;
}
...
if ($action == 'langpicker_up') {
	...
	header('Location: '.$_SERVER["PHP_SELF"].'?tab=langpicker');
	exit;
}
...
if ($action == 'update_langpicker' && !GETPOST('cancel', 'alpha') && GETPOST('token', 'none') === newToken()) {
	dolibarr_set_const($db, 'LANG_PICKER_ADD_TO_LOGIN_PAGE', GETPOSTINT('LANG_PICKER_ADD_TO_LOGIN_PAGE'), 'chaine', 0, '', $conf->entity);
	...
	dolibarr_set_const($db, 'LANG_PICKER_HIDDEN', GETPOSTINT('LANG_PICKER_HIDDEN'), 'chaine', 0, '', $conf->entity);
	dolibarr_set_const($db, 'LANG_PICKER_POSITION', GETPOST('LANG_PICKER_POSITION', 'alphanohtml'), 'chaine', 0, '', $conf->entity);
...
if ($action == 'upload_terms' && GETPOST('token', 'none') === newToken()) {
	...
	header('Location: '.$_SERVER["PHP_SELF"].'?tab=rubis');
	exit;
}
...
$taburl = DOL_URL_ROOT.'/slycustom/admin/setup.php';
$head = array();
...
if ($tab == 'general') {
	...
	if ($action == 'edit') {
		...
		print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?tab=general&action=edit&token='.newToken().'">'.$langs->trans("Modify").'</a>';
		print '</div></td></tr>';
	}
...
if ($tab == 'langpicker') {
	...
	if ($action == 'edit_langpicker') {
		...
		print '<table class="noborder centpercent">';
		print '<tr class="liste_titre"><td class="titlefield">'.$langs->trans("Parameter").'</td><td>'.$langs->trans("Value").'</td></tr>';
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_ADD_TO_LOGIN_PAGE").'</td><td>';
		print $form->selectyesno('LANG_PICKER_ADD_TO_LOGIN_PAGE', getDolGlobalString('LANG_PICKER_ADD_TO_LOGIN_PAGE', 0), 1);
		print '</td></tr>';
		...
		print '</td></tr>';
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_HIDDEN").'</td><td>';
		print $form->selectyesno('LANG_PICKER_HIDDEN', getDolGlobalString('LANG_PICKER_HIDDEN', 0), 1);
		print '</td></tr>';
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_POSITION").'</td><td>';
		print '<input type="number" name="LANG_PICKER_POSITION" class="flat width50" value="'.dol_escape_htmltag(getDolGlobalString('LANG_PICKER_POSITION', 130)).'" min="0" max="999">';
		print '</td></tr>';
		print '</table>';
		...
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_ADD_TO_LOGIN_PAGE").'</td><td>'.(getDolGlobalString('LANG_PICKER_ADD_TO_LOGIN_PAGE') ? $langs->trans("Yes") : $langs->trans("No")).'</td></tr>';
		...
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_HIDDEN").'</td><td>'.(getDolGlobalString('LANG_PICKER_HIDDEN') ? $langs->trans("Yes") : $langs->trans("No")).'</td></tr>';
		print '<tr class="oddeven"><td>'.$langs->trans("LANG_PICKER_POSITION").'</td><td>'.dol_escape_htmltag(getDolGlobalString('LANG_PICKER_POSITION', 130)).'</td></tr>';
		print '<tr><td colspan="2"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?tab=langpicker&action=edit_langpicker&token='.newToken().'">'.$langs->trans("Modify").'</a></td></tr>';
	}
	print '</table>';
	print '</div>';
	...
	if (count($langpicker->lines) > 0) {
		...
		foreach ($langpicker->lines as $line) {
			...
			$up = $i > 0 ? '<a href="'.$_SERVER["PHP_SELF"].'?tab=langpicker&action=langpicker_up&id='.$line->id.'&token='.newToken().'">'.img_up('default', 0, 'imgupforline').'</a>' : '';
			$down = $i < $nb - 1 ? '<a href="'.$_SERVER["PHP_SELF"].'?tab=langpicker&action=langpicker_down&id='.$line->id.'&token='.newToken().'">'.img_down('default', 0, 'imgdownforline').'</a>' : '';
			print $up.' '.$down.'</td><td class="right">';
			print '<a href="'.$_SERVER["PHP_SELF"].'?tab=langpicker&action=langpicker_delete&id='.$line->id.'&token='.newToken().'" class="reposition">'.img_delete($langs->trans("Delete")).'</a>';
			print '</td></tr>';
			$i++;
		}
	}
