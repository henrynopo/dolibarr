...
class ActionsSlycustom
{
	...
	public function formBuilddocOptions($parameters, $object)
	{
		...
		$this->resprints .= '<script nonce="'.getNonce().'">document.addEventListener("DOMContentLoaded",function(){var cb=document.getElementById("sly_add_terms_cb");var block=document.getElementById("sly_terms_template_block");function sync(){if(block)block.style.display=cb&&cb.checked?"inline":"none";}if(cb){cb.addEventListener("change",sync);sync();}var form=document.getElementById("builddoc_form");if(form){form.addEventListener("submit",function(){if(cb&&cb.checked){var m=window.location.search.match(/[?&]id=(\d+)/);var id=m?m[1]:"";if(id){form.action="'.dol_escape_js(DOL_URL_ROOT.'/slycustom/builddoc_order.php').'?id="+id;}}},false);}});</script>';
	 ...
	 * When using zero-core-modification flow, sales terms are handled by slycustom/builddoc_order.php
	...
	public function printTopRightMenu($parameters, &$object, &$action)
	{
		...
		foreach ($langpicker->lines as $lang) {
			...
			$url = DOL_URL_ROOT.'/slycustom/set_lang.php?lang_code='.urlencode($lang->lang_code).'&backtopage='.$backtopage;
	...
	/**
	 * Hook getLoginPageOptions: add language select on login page
	 *
	 * @param array        $parameters Hook parameters
	 * @param CommonObject $object     Object
	 * @param string       $action     Action
	...
	public function getLoginPageOptions($parameters, &$object, &$action)
	{
		...
		if (!getDolGlobalString('LANG_PICKER_ADD_TO_LOGIN_PAGE')) {
			return 0;
		}
		...
		$langs->load("slycustom@slycustom");
		// Use configured default language on login page for all users when no lang_code in request
		$selected_lang = GETPOST('lang_code', 'alphanohtml') ?: getDolGlobalString('LANG_PICKER_LOGIN_PAGE_DEFAULT_LANG', 'en_US');
		// Same structure as Login/Password rows for alignment and icon: trinputlogin > tagtd.tdinputlogin > icon + control
		// forcecombo=0 so select2 is used: dropdown opens downward like Dolibarr style, and options show flag icons via data-html/templateResult
		...
		$this->resprints .= $formadmin->select_language($selected_lang, 'lang_code', 0, null, $langs->trans('SelectLanguage'), 0, 0, 'minwidth150', 0, 0);
		$this->resprints .= '</div></div>';
		return 0;
	}

	/**
	 * Hook getLoginPageExtraOptions: when user changes language on login page, reload page with that lang so labels update
	 *
	 * @param array        $parameters Hook parameters
	 * @param CommonObject $object     Object
	 * @param string       $action     Action
...
(function() {
	...
	function bindLangReload() {
		...
		if (window.jQuery && window.jQuery(sel).data("select2")) {
			window.jQuery(sel).off("select2:select").on("select2:select", onLangChange);
		}
	}
	function showFlagInSelection() {
		var sel = document.getElementById("lang_code");
		if (!sel) return;
		var opt = sel.options[sel.selectedIndex];
		if (!opt || !opt.getAttribute("data-html")) return;
		var html = opt.getAttribute("data-html");
		if (typeof htmlEntityDecodeJs === "function") html = htmlEntityDecodeJs(html);
		var rendered = document.querySelector("#lang_code + .select2-container .select2-selection__rendered");
		if (rendered) rendered.innerHTML = html;
	}
	if (document.readyState === "loading") {
		document.addEventListener("DOMContentLoaded", function() { bindLangReload(); setTimeout(showFlagInSelection, 300); });
	} else {
		bindLangReload();
		setTimeout(showFlagInSelection, 300);
	}
	setTimeout(function() { bindLangReload(); showFlagInSelection(); }, 500);
})();
</script>';
		return 0;
	}
	...
	/**
	 * Hook afterLogin: apply language chosen on login page
	 *
	 * @param array        $parameters Hook parameters
	...
	public function afterLogin($parameters, &$object, &$action)
	{
		...
		if ($lang_code === '' || $lang_code === null) {
			$lang_code = getDolGlobalString('LANG_PICKER_LOGIN_PAGE_DEFAULT_LANG', 'en_US');
		}
		if ($lang_code !== '' && (empty($user->conf->MAIN_LANG_DEFAULT) || $user->conf->MAIN_LANG_DEFAULT != $lang_code)) {
