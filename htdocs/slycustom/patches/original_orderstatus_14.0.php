...
/**
 * SLY 14.0 原始 orderstatus.php（从 sly14.0-fourn.patch 提取）
 * 功能：Search Order — 输入销售订单号，显示关联的 SO/PO 及文档、关联对象
 * 放置位置：htdocs/fourn/commande/orderstatus.php
 *
 * 注意：原版存在若干问题，在 22.0 直接使用需修正：
 * - $id 未定义即用于 restrictedArea
 * - 未搜索或无结果时 $obj 为 null，仍会 fetch 导致报错
 * - SO_ATA(c.rowid) 可能为 SLY 自定义 SQL 函数，需确认数据库是否存在
 * - $usercancreate 未定义，应为 $usercancreate_so / $usercancreate_po
 */
